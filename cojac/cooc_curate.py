...
def parse_args():
    ...
    argparser = argparse.ArgumentParser(
        ...
        description="helps determining specific mutations and cooccurences by querying CoV-Spectrum",
    ...
    argparser.add_argument(
        ...
        action="store_false",
        dest="collapse",
        help="consider each sublineage separately (e.g.: count separately BA.1, BA.1.1, BA.2, BA.3, etc. instead of counting B.1.529*)",
    )
    argparser.add_argument(
        "-c",
        "--ansi",
    ...
    argparser.add_argument(
        "VOC", nargs="+", metavar="VOC", help="per VOC description YAML file(s)"
    )

    # TODO YAML output for colourmut and pubmut

    args = argparser.parse_args()

    if args.amp:
        assert os.path.isfile(args.amp), f"cannot find amplicons YAML file {args.amp}"
    for yam in args.VOC:
        assert os.path.isfile(yam), f"cannot find VOC description YAML file {align}"
...
def main():
    ...
    for voc_fname in args.VOC:
        ...
        if len(sublineages) > 1 and args.collapse:
        ...
        # mutations
        if args.domuts or amplicons is None:
            print("\nmutations:")
